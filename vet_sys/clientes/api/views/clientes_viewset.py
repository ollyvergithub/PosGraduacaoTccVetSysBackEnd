...
class ClientesViewSet(viewsets.ModelViewSet):
    ...
    def gerar_estatisticas_por_ano(self, request):
        ...
        for i in range(1, 13):
            result = Cliente.objects.filter(criado_em__month=i, criado_em__year=ano).count()
        ...
        ax.set(xlabel='Meses', ylabel='Quantidade', title=f'Número de clientes por mês em {ano}')
        ax.grid()
    ...
    def gerar_estatisticas_por_mes_e_ano(self, request):
        ...
        result = Cliente.objects.filter(criado_em__month=mes, criado_em__year=ano).count()
        ...
        ax.set(xlabel='Mes', ylabel='Quantidade', title=f"Número de clientes em {meses[mes]} de {ano}")
        ax.grid()
