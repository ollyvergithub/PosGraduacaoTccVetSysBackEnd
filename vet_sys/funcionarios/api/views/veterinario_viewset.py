...
class VeterinariosViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        crmv = self.request.query_params.get('crmv')
        if crmv:
            qs = qs.filter(crmv__unaccent__icontains=crmv)
    ...
    def gerar_relatorio_pdf(self, request):
        from vet_sys.funcionarios.api.serializers.validation_serializers.veterinario_validate_serializer import GerarRelatorioPdfValidateSerializer
        ...
        uuids_veterinarios = self.request.data.get('uuids_veterinarios', None)

        veterinarios = Veterinario.objects.filter(uuid__in=uuids_veterinarios)

        html_string = render_to_string(
            'pdf/veterinarios/pdf.html',
            {'veterinarios': veterinarios, 'usuario_logado': usuario_logado, 'data_atual': data_atual}).encode(
            encoding="UTF-8")
        ...
        response['Content-Disposition'] = 'filename="relatorio-veterinarios.pdf"'
